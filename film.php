...
$resultMovie = $pdo->prepare($sql); 

// this binds the query of resultMovie by looking
// for :id in the $sql and safely replaces (binds) it with the data stored in $id
$resultMovie->bindValue(':id', $id, PDO::PARAM_STR);

//this then executes the resulty
$resultMovie->execute();

// fetch() will retrieve a single row
$row_of_id = $resultMovie->fetch(PDO::FETCH_ASSOC);

//REVIEWS //REVIEWS //REVIEWS 

//this says if when it checks the movie and it is set and is not NULL
//then the varable movie is equal to movie from the database
if (isset($_GET['movie'])) {
  $movie= $_GET['movie'];
} else {
  $movie='Avatar';// if no movie is returned then the
  // default page to show will be one with the movie avatar
}
//then echo the movie variable in special characters  will return as a function
//with all the conversions made
echo $movie;
$movie =htmlspecialchars($movie,ENT_QUOTES, 'UTF-8');

//prepares the movie result set
$movie_sql = "SELECT * FROM reviews ";
$resultmovienamer = $pdo -> prepare($movie_sql);
// this prepares and stores the SQL statement in 
  //a variable called $resultMovie -->

$resultmovienamer -> execute();
//this then executes the resulty
?>
...
<!-- i then open the genre and film containers which contains the css styling from style.css
...
<div class="genre-container">
 ...
 the movie name, the movie cover image, the director, the year,
 the main actors, the long synopsis, the average rating, and the 
 button for adding a review -->
  	<?php
  echo '<img src="images/covers/'.$row_of_id['cover'].'"/>';
  echo '<t8>' . $row_of_id['name'] . '</t8><br><br>' ;
  echo '<t9>' ,'directed by: ','</t9>';
	echo '<t9>' . $row_of_id['director'] . '</t9>' ;
  echo '<t9>, </t9>' ;
	echo '<t9>' . $row_of_id['year'] . '</t9><br>';
	echo '<t9>' ,'staring: ','</t9>';
	echo '<t9>'. $row_of_id['mainactors'] . '</t9><br>' ;
	echo '<p> <font size="+0.5">'. $row_of_id['synopsislong'] . '</p></font><br>' ;
	echo '<t9>' ,'average rating:','</t9><br>';
	echo '<t10>' . $row_of_id['average rating'] . '</t10>';
                 echo "<t10>/5  </t10>";
                 echo '<i class="fas fa-star">', '</i>';
                 echo "<br><br><br><br><br>";

  echo '<a href="add_input.php" style="float:left" class="button">add a review</a><br><br><br>';
  echo '<t6>', 'current reviews', '<t6>';
  echo '</div>';
    ?>
...
<!-- this begins a php foreach loop whih states that
 for each of the results that the loops will create
 a row which will echo:
 the reviews container, the reviewer name, the movie name, the rating,
 and the review -->
 <?php 
  // Generate output for each row that was returned (there should only be one row though!)
  foreach($resultmovienamer as $row) {
    echo '<div class="reviews-container">';
    echo '<t11><b>' . $row['reviewername'] . '</b></t11><br>';
    echo "<t12>  </t12> ";
    echo '<t9>' . $row['movie'] . '</t9></br>' ;
     // information 
    echo "<t12>rating:</t12> ";
    echo '<t12>' . $row['rating'] . '</t12>';
    echo "<t12>/5</t12> <br>";
    echo '<p>' . $row['review']. '</p>'; // pre-formatted information to display
    echo '</div>';
  } ?>
</div>
